...
import {
  ...
  Droplets, FileText, Users, Settings, LogOut,
  ChefHat, Menu, X
} from "lucide-react";
...
export default function AdminLayout() {
    ...
    <div style={{ display: "flex", flexDirection: "column", height: "100%" }}>
      ...
      <div style={{ padding: "20px 16px 24px", borderBottom: "1px solid rgba(255,255,255,0.08)" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          <div style={{ width: 40, height: 40, background: "var(--brand)", borderRadius: 12, display: "flex", alignItems: "center", justifyContent: "center" }}>
            <ChefHat size={20} color="white" />
          </div>
          <div>
            <div style={{ fontFamily: "'Playfair Display', serif", fontWeight: 700, color: "white", fontSize: 16 }}>La Touche D</div>
            <div style={{ fontSize: 11, color: "rgba(255,255,255,0.4)" }}>Administration</div>
          </div>
        </div>
        <div style={{ marginTop: 16, padding: "10px 12px", background: "rgba(255,255,255,0.06)", borderRadius: 10 }}>
          <div style={{ color: "white", fontWeight: 600, fontSize: 13 }}>{userData?.name || "Admin"}</div>
          <div style={{ fontSize: 11, color: "rgba(255,255,255,0.4)", marginTop: 2 }}>
            <span style={{ background: "var(--brand)", color: "white", padding: "1px 8px", borderRadius: 10, fontSize: 10 }}>Administrateur</span>
          </div>
        </div>
      </div>
  ...
  return (
    <div style={{ display: "flex", minHeight: "100vh" }}>
      {/* Desktop sidebar */}
      <aside style={{
        width: 240, background: "var(--gray-900)", flexShrink: 0,
      ...
      {sidebarOpen && (
        <div style={{ position: "fixed", inset: 0, zIndex: 300, display: "flex" }}>
          ...
          <aside style={{ width: 260, background: "var(--gray-900)", position: "relative", zIndex: 1 }}>
            <button style={{ position: "absolute", top: 16, right: 16, background: "none", border: "none", cursor: "pointer", color: "white" }} onClick={() => setSidebarOpen(false)}>
              <X size={20} />
            </button>
            <SidebarContent />
          </aside>
      ...
      <main style={{ flex: 1, minHeight: "100vh", display: "flex", flexDirection: "column" }}>
        ...
        <header style={{
          ...
          <button
            ...
            style={{ background: "rgba(255,255,255,0.1)", border: "none", borderRadius: 8, padding: "8px", cursor: "pointer", color: "white" }}
          >
            <Menu size={20} />
          </button>
          <div style={{ fontFamily: "'Playfair Display', serif", fontWeight: 700, color: "white", fontSize: 16 }}>La Touche D</div>
          <div style={{ marginLeft: "auto", fontSize: 12, color: "rgba(255,255,255,0.5)" }}>{userData?.name}</div>
        </header>
